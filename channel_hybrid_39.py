...
class DataPreparation(object):
    ...
    def split_dataset(self):
        ...
        real_path = '/media/sdc/djd/to_public_repository/full_stacks/'
        real_name = []
        for i in sorted(os.listdir(real_path)):
            if i.split('.')[1] == 'tiff':
                ...
                b = a[:-9]
                c = b + 'cellmask.tiff'
                tiff_path = '/media/sdc/djd/to_public_repository/full_stacks//' + i
                mask_path = '/media/sdc/djd/to_public_repository/cell_masks/' + c
                #             if b in info_list:
        ...
        self.split_list['test'] = self.tiffPath_gtLabelList_brca[num_trainData:]
...
model = Channel_hybrid(in_channels=39)
# for param in model.decoding.parameters():
...
optimizer = torch.optim.Adam(
    params_optimize, lr=0.001, betas=(0.9, 0.999), eps=1e-08,
...
writer = SummaryWriter('log_dir/cean39/')
epoch_dice = 0
...
for epoch in range(num_epochs):
    ...
    for phase in phase_list[:2]:
        ...
        running_num = 0.0
        if phase == 'train':
            exp_lr_scheduler.step(epoch_dice)
        ...
        for x, y in dataloader:
            ...
            with torch.set_grad_enabled(phase == 'train'):
                ...
                pred_dice[pred_dice >= 0.5] = 1
                pred_dice[pred_dice < 0.5] = 0
        ...
        print('epoch!')
        # 这里定义的时候 training 和 testing  的loss都一样吗？
        epoch_loss = running_loss / running_num
        epoch_dice = running_dice / running_num
        ...
        if 'valid' in phase_list:
            ...
            test_phase = 'test'
        if phase == test_phase and epoch_dice > best_dice:
            best_dice = epoch_dice
            #             if os.path.exists('./channel_trans_gap/') == False:
            #                 os.mkdir('./channel_trans_gap/')
            #             if os.path.exists(bestModel_path):
            #                 os.remove(bestModel_path)
            bestModel_path = 'cean39_model/best' + str(epoch) + '.pt'
        ...
        if phase == 'test':
            ...
            writer.add_scalar('Dice/test', epoch_dice, epoch)
